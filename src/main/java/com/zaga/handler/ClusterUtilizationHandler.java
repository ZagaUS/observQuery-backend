...
public class ClusterUtilizationHandler {
        ...
        public List<ClusterResponse> getAllClusterByDateAndTime(
                ...
                if (from != null && to != null) {
                        result = executeAggregationPipeline(
                                        collection,
                                        from,
                                        to,
                                        clusterName,
                        ...
                        result = executeAnotherLogic(collection, from, minutesAgo, clusterName, nodeName);
                } else {
                        System.out.println(
        ...
        public List<ClusterResponse> executeAggregationPipeline(
                ...
                // matchPipeline with clusterName,NodeName , if Nodename == null then match with
                // clustername only
                List<Document> aggregatedDocuments = matchPipeline(pipeline, clusterName, nodeName);
                // groupPipeline with clusterName,NodeName , if Nodename == null then group by
                // only clustername
        ...
        public List<ClusterResponse> resultData(AggregateIterable<Document> aggregationResult) {
                ...
                Double cpuUsageResponse = 0.00;
                Double memoryUsageResponse = 0.00;
                Double memoryAvailableResponse = 0.00;
                Double fileSystemUsageResponse = 0.00;
                Double fileSystemAvailableResponse = 0.00;
                System.out.println("-------------[START ITERATE DOCUMENT]------------");
                for (Document document : aggregationResult) {

                        cpuUsageResponse += document.getDouble("cpuUsage");
                        memoryUsageResponse += document.getDouble("memoryUsage");
                        memoryAvailableResponse += document.getDouble("memoryAvailable");
                        fileSystemUsageResponse += document.getDouble("fileSystemUsage");
                        fileSystemAvailableResponse += document.getDouble("fileSystemAvailable");
                        System.out.println(document.getDouble("cpuUsage") + " - " + document.getDouble("memoryUsage")
                                        + " - " + document.getDouble("memoryAvailable") + " - "
                                        + document.getDouble("fileSystemUsage") + " - " + document.getDouble("fileSystemAvailable"));

                }
                System.out.println("-------------[START ITERATE DOCUMENT -END]------------");
                if (cpuUsageResponse > 0.00) {
                        ClusterResponse clusterResponse = new ClusterResponse();
                        clusterResponse.setCpuUsage(cpuUsageResponse);
                        clusterResponse.setMemoryUsage(memoryUsageResponse);
                        clusterResponse.setMemoryAvailable(memoryAvailableResponse);
                        clusterResponse.setFileSystemUsage(fileSystemUsageResponse);
                        clusterResponse.setFileSystemAvailable(fileSystemAvailableResponse);
                ...
                groupPipeLine = Arrays.asList(new Document("$group",
                                new Document("_id",
                                                ...
                                                .append("avgMemoryAvailable",
                                                                new Document("$avg",
                                                                                new Document("$divide", Arrays
                                                                                                .asList("$memoryAvailable",
                                                                                                                1024L))))
                                                .append("avgFileSystemCapacity",
                                                                new Document("$avg",
                                                                                new Document("$divide", Arrays
                                                                                                .asList("$fileSystemCapacity",
                                                                                                                1024L * 1024L * 1024L))))
                                                .append("avgFileSystemUsage",
                                                                new Document("$avg",
                                                                                new Document("$divide", Arrays
                                                                                                .asList("$fileSystemUsage",
                                                                                                                1024L * 1024L * 1024L))))
                                                .append("avgFileSystemAvailable",
                                                                new Document("$avg",
                                                                                new Document("$divide", Arrays
                                                                                                .asList("$fileSystemAvailable",
                                                                                                                1024L * 1024L * 1024L))))),
                                new Document("$project",
                                                new Document("cpuUsage", "$avgCpuUsage")
                                                                ...
                                                                .append("fileSystemCapacity",
                                                                                "$avgFileSystemCapacity")
                                                                .append("fileSystemUsage",
                                                                                "$avgFileSystemUsage")
                                                                .append("fileSystemAvailable",
                                                                                "$avgFileSystemAvailable")));

                // List<Document> aggregatedList = Stream.concat(pipeline.stream(),
                // groupPipeLine.stream()).toList();
                List<Document> aggregatedList = Stream.concat(pipeline.stream(), groupPipeLine.stream())
        ...
        public List<Document> matchPipeline(List<Document> pipeline, String clusterName, String nodeName) {
                ...
                if (nodeName != null) {
                        System.out.println(
                                        "------------[MATCH PIPELINE / IF]---------- " + clusterName + "  " + nodeName);
                        matchPipeLine = Arrays.asList(new Document("$match",
                                        new Document("clusterName", clusterName)
                                                        .append("nodeName", nodeName)));
                } else {
                        System.out.println("------------[MATCH PIPELINE / ELSE]---------- " + clusterName + "  "
                                        + nodeName);
                        matchPipeLine = Arrays.asList(new Document("$match",
                                        new Document("clusterName", clusterName)));
                }
                // List<Document> matchFilter = Stream.concat(pipeline.stream(),
                // matchPipeLine.stream()).toList();
                List<Document> matchFilter = Stream.concat(pipeline.stream(), matchPipeLine.stream())
                                .collect(Collectors.toList());
                return matchFilter;
        }
}
